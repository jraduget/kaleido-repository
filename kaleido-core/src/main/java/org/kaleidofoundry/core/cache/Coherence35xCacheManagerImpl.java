...
class Coherence35xCacheManagerImpl extends AbstractCacheManager {
   ...
   /** internal logger */
   private static final Logger LOGGER = LoggerFactory.getLogger(Coherence35xCacheManagerImpl.class);
    ...
    * @param context
    */
   public Coherence35xCacheManagerImpl(final RuntimeContext<CacheManager> context) {
	this(null, context);
   }
    ...
    * @param context
    */
   public Coherence35xCacheManagerImpl(final String configuration, final RuntimeContext<CacheManager> context) {
   ...
   public String getMetaInformations() {
	return "coherence-3.x - [3.0.x -> 3.5.3]";
   }
   ...
   public <K extends Serializable, V extends Serializable> Cache<K, V> getCache(@NotNull final String name) {
	...
	if (cache == null) {
	   ...
	   cache = new Coherence35xCacheImpl(name, coherenceCache);
   ...
   public void destroy(final String cacheName) {
	final Coherence35xCacheImpl<?, ?> cache = (Coherence35xCacheImpl) cachesByName.get(cacheName);
	if (cache != null) {
	   cache.destroy();
