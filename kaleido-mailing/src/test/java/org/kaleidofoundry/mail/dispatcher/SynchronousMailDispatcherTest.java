...
public class SynchronousMailDispatcherTest extends AbstractSynchronousMailDispatcherTest {
    ...
    public void setup() {
	  ...
	  ServerOptions options = new ServerOptions(new String[] {
		    ConfigurationFactory.getRegistry().get("local-dumbster-session").getString("mailing.sessions.mySession-ssl.smtp.port") });
    ...
    public static void mailWithAttachmentsAssertions(SmtpServer server, int mailIndex) {
	  ...
	  assertTrue(
		    server.getMessage(mailIndex).getBody().contains("<line>this is an email attachment with some specials characters</line>"));
	  ...
	  assertTrue(server.getMessage(mailIndex).getBody()
		    .contains("R0lGODlhvAIsAfcAAHPLLiEkIqQnKhkoKjNKYjNUJ9Amc3FrJJAmarInV/LMmP4mavvmsycoHTYo"));
